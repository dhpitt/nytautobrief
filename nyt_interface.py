...
def retrieve_and_summarize(section = 'science', length = 5):
	...
	for story in top_stories[2:]:
		blurb += story['title'] + '. ' + story['abstract'] + ' ' + story['multimedia'][0]['caption']

	text = openai.Completion.create(
	  model="text-davinci-003",
	  prompt=f"Summarize the following into {length} sentences for a layperson who is a science enthusiast: " + blurb,
